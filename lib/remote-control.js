...
export async function runRemoteControlSession(params) {
  ...
  params.io.stdout.write([
    `Remote relay session ready.`,
    `Session ID: ${relaySession.sessionId}`,
    `Client connection URL: ${relaySession.clientConnectionUrl}`,
    `Pairing token: ${relaySession.pairingToken}`,
    `Expires at: ${relaySession.expiresAt}`,
    '',
  ].join('\n'));
