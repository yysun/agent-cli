  ...
  switch (provider) {
    ...
    case 'azure':
      return {
        ...
        deployment: requireEnvironmentVariable(environment, 'AZURE_OPENAI_DEPLOYMENT'),
 ...
 * @param {NodeJS.ProcessEnv} [environment]
 */
function resolveRuntimeSettings(environment = process.env) {
  const provider = String(environment.LLM_PROVIDER ?? 'openai').trim();

  if (!SUPPORTED_PROVIDERS.has(provider)) {
    throw new Error(`Unsupported LLM provider: ${provider}`);
  }

  const model = String(environment.LLM_MODEL ?? DEFAULT_MODELS[provider] ?? '').trim();
  ...
  return {
    ...
    model,
    providers: {
      [provider]: resolveProviderConfig(provider, environment),
    },
  };
}
 ...
 * }} params
 */
export async function runChatTurn({ chat, userMessage, systemPrompt, skillInventory }) {
  ...
  try {
    const result = await respondWithTools({
      initialState: {
        conversationMessages: [...chat.messages, pendingUserMessage],
        finalText: '',
      },
    ...
    return {
      ...
      messages: result.state.conversationMessages,
    };
  } finally {
