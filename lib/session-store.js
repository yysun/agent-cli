 ...
 *
 * Purpose:
 * - Persist current-chat metadata and completed conversation turns under `./agent/sessions`.
 *
 * Key features:
...
import { CHAT_DIRECTORY, CURRENT_CHAT_PATH } from './paths.js';
...
export async function loadRequestedChat({ newChat }) {
  ...
  const currentPointer = await readJson(
    CURRENT_CHAT_PATH,
  ...
  const chat = await readJson(
    chatPath,
    `Current chat file is missing: ${chatPath}`,
...
export async function persistCompletedChat({ chat, messages }) {
  ...
  await writeJsonAtomic(buildChatPath(chat.id), persistedChat);
