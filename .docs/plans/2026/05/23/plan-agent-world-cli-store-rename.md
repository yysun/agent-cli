...
Implement `cli/src/agent-world-cli.ts` as a small command dispatcher over `createAgentWorldRuntime`. The CLI should stay local-first and avoid duplicating runtime behavior: parsing turns flags and positional arguments into world-runtime API calls, and output stays JSON by default for deterministic shell use. Add an interactive shell on top of the same dispatcher so the REPL path cannot drift from one-shot behavior.
...
flowchart TD
  ...
  B --> F["JSON output"]
  H["interactive stdin"] --> I["slash/plain-text adapter"]
  I --> B
  G["agent-cli / relay"] --> D
```
...
- Interactive mode can accidentally diverge from one-shot command behavior unless it reuses the same dispatcher.
- Plain-text sends in interactive mode can require provider credentials; tests should use queue mode or non-send commands unless live-provider coverage is intentional.
