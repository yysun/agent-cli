...
Interactive mode belongs in `cli/src/cli-shell.ts` because it is CLI orchestration, not runtime execution. It should reuse `resolveEffectiveAgentConfig`, `loadProjectSystemPrompt`, `loadSkillInventory`, `createTurnExecutor`, and session-store helpers so prompt turns behave like normal CLI turns.
