...
1. Agent CLI must keep a built-in system prompt separate from project `AGENTS.md` content.
2. Agent CLI must keep the current layering model in which the built-in prompt, optional project prompt, and skill inventory remain distinct prompt contributions.
...
11. Existing project `AGENTS.md` loading behavior must remain unchanged.
...
2. `loadProjectSystemPrompt()` behavior remains unchanged.
3. `buildSkillInventoryMessage()` behavior remains unchanged.
4. Runtime prompt layering still uses the built-in prompt plus optional project prompt plus optional skill inventory as separate system-message contributions.
5. Unit tests verify the upgraded built-in prompt and preserve the separation between built-in prompt loading and project prompt loading.
