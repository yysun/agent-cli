...
- Storage: use `.agent-world` only. No `.chats` compatibility paths, no `.agent-world/worlds`, no registry, no `world.json`, no `agents`, and no `agent.json`.
...
- `.env`: credentials, `AGENT_CLI_*` runtime defaults, and optional workspace selection only.
